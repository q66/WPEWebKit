...
static cairo_device_t* cairoDevice()
{
    ...
    std::call_once(s_flag,
        [&] {
            ...
            s_device = cairo_egl_device_create(platformDisplay.eglDisplay(), context->context());
        });
...
std::unique_ptr<ImageBufferCairoGLSurfaceBackend> ImageBufferCairoGLSurfaceBackend::create(const Parameters& parameters, const ImageBufferCreationContext&)
{
    IntSize backendSize = calculateBackendSize(parameters);
...
void ImageBufferCairoGLSurfaceBackend::getPixelBuffer(const IntRect& srcRect, PixelBuffer& destination)
{
    auto surface = cairoGLSurfaceCopyToImageSurface(m_surfaces[0].get());
    ImageBufferBackend::getPixelBuffer(srcRect, cairo_image_surface_get_data(surface.get()), destination);
}
...
RefPtr<NativeImage> ImageBufferCairoGLSurfaceBackend::copyNativeImage(BackingStoreCopy copyBehavior)
{
    switch (copyBehavior) {
